...
def test_committed_fixture_coverage_keeps_remaining_families_visible() -> None:
    ...
    assert report.complete is False
    assert report.complete_family_count == 2
    assert report.incomplete_family_count == 10
    assert report.missing_required_artifact_count == 40
...
def test_fixture_coverage_json_round_trip() -> None:
    ...
    assert payload["complete_family_count"] == 2
...
def test_fixture_coverage_script_writes_json_and_markdown(tmp_path: Path) -> None:
    ...
    code = main(["--output", str(output), "--markdown", str(markdown), "--strict", "--min-complete-families", "2"])
